/*! Amaze UI v2.0.0 ~ Old IE Fucker | by Amaze UI Team | (c) 2014 AllMobilize, Inc. | Licensed under MIT | 2014-12-05T02:12:50 UTC */
(function e(t, n, r) {
  function s(o, u) {
    if (!n[o]) {
      if (!t[o]) {
  ...
  2: [
    function(require, module, exports) {
      (function(global) {
		 ...
		 * @param {number}
		 *            wait Function execution threshold in milliseconds
		 * @param {bool}
		 *            immediate Whether the function should be called at the
		 *            beginning of the delay instead of the end. Default is
		 *            false.
        ...
        UI.utils.rAF = (function() {
          return window.requestAnimationFrame ||
            ...
            function(callback) {
              return window.setTimeout(callback, 1000 / 60); // shoot for 60
																// fps
            };
        })();
        ...
        /**
		 * https://github.com/cho45/micro-template.js (c) cho45
		 * http://cho45.github.com/mit-license
		 */
        /* jshint -W109 */
          ...
          if (!me.cache[id]) {
            me.cache[id] = (function() {
              ...
              var body = ('try { ' + (me.variable ?
                  ...
                  'with (this.stash) { ') +
                "this.ret += '" +
                string.replace(/<%/g, '\x11').replace(/%>/g, '\x13'). // if
																		// you
																		// want
																		// other
																		// tag,
      ...
      (function(global) {
        ...
        Dropdown.prototype.events = function() {
          ...
          /*
			 ...
			 * 
			 * if (trigger === 'click') { $toggle.on('click.' + eventNS,
			 * $.proxy(this.toggle, this)) }
			 * 
			 * if (trigger === 'focus' || trigger === 'hover') { var eventIn =
			 * trigger == 'hover' ? 'mouseenter' : 'focusin'; var eventOut =
			 * trigger == 'hover' ? 'mouseleave' : 'focusout';
			 * 
			 * this.$element.on(eventIn + '.' + eventNS, $.proxy(this.open,
			 * this)) .on(eventOut + '.' + eventNS, $.proxy(this.close, this)); } }
			 */
  ...
  8: [
    function(require, module, exports) {
      (function(global) {
        ...
        /*
		 * jQuery FlexSlider v2.2.2 Copyright 2012 WooThemes Contributing
		 * Author: Tyler Smith
		 */
                ...
                if (methods.pauseInvisible.visProp) {
                  ...
                  document.addEventListener(evtname, function() {
                    if (methods.pauseInvisible.isHidden()) {
                      ...
                      if (slider.started) slider.play(); // Initiated before,
															// just play
                      else(slider.vars.initDelay > 0) ? setTimeout(slider
                        .play, slider.vars.initDelay) : slider.play(); // Didn't
																		// init
																		// before:
																		// simply
																		// init
        ...
        $.flexslider.defaults = {
          namespace: "am-", // {NEW} String: Prefix string attached to the class
							// of every element generated by the plugin
          selector: ".am-slides > li", // {NEW} Selector: Must match a simple
										// pattern. '{container} > {slide}' --
										// Ignore pattern at your own peril
          animation: "slide", // String: Select your animation type, "fade" or
								// "slide"
          easing: "swing", // {NEW} String: Determines the easing method used
							// in jQuery transitions. jQuery easing plugin is
							// supported!
          ...
          animationLoop: true, // Boolean: Should the animation loop? If false,
								// directionNav will received "disable" classes
								// at either end
          smoothHeight: false, // {NEW} Boolean: Allow height of the slider to
								// animate smoothly in horizontal mode
          startAt: 0, // Integer: The slide that the slider should start on.
						// Array notation (0 = first slide)
          ...
          animationSpeed: 600, // Integer: Set the speed of animations, in
								// milliseconds
          initDelay: 0, // {NEW} Integer: Set an initialization delay, in
						// milliseconds
          randomize: false, // Boolean: Randomize slide order
          fadeFirstSlide: true, // Boolean: Fade in the first slide when
								// animation type is "fade"
          thumbCaptions: false, // Boolean: Whether or not to put captions on
								// thumbnails when using the "thumbnails"
								// controlNav.
          ...
          pauseOnHover: false, // Boolean: Pause the slideshow when hovering
								// over slider, then resume when no longer
								// hovering
          ...
          useCSS: true, // {NEW} Boolean: Slider will use CSS3 transitions if
						// available
          touch: true, // {NEW} Boolean: Allow touch swipe navigation of the
						// slider on touch-enabled devices
          video: false, // {NEW} Boolean: If using video in the slider, will
						// prevent CSS3 3D Transforms to avoid graphical
						// glitches

          // Primary Controls
          controlNav: true, // Boolean: Create navigation for paging control of
							// each slide? Note: Leave true for manualControls
							// usage
          directionNav: true, // Boolean: Create navigation for previous/next
								// navigation? (true/false)
          prevText: "Previous", // String: Set the text for the "previous"
								// directionNav item
          nextText: "Next", // String: Set the text for the "next" directionNav
							// item
          ...
          manualControls: "", // {UPDATED} jQuery Object/Selector: Declare
								// custom control navigation. Examples would be
								// $(".flex-control-nav li") or "#tabs-nav li
								// img", etc. The number of elements in your
								// controlNav should match the number of
								// slides/tabs.
          sync: "", // {NEW} Selector: Mirror the actions performed on this
					// slider with another slider. Use with care.
          asNavFor: "", // {NEW} Selector: Internal property exposed for turning
						// the slider into a thumbnail navigation for another
						// slider

          // Carousel Options
          itemWidth: 0, // {NEW} Integer: Box-model width of individual carousel
						// items, including horizontal borders and padding.
          itemMargin: 0, // {NEW} Integer: Margin between carousel items.
          minItems: 1, // {NEW} Integer: Minimum number of carousel items that
						// should be visible. Items will resize fluidly when
						// below this.
          maxItems: 0, // {NEW} Integer: Maxmimum number of carousel items that
						// should be visible. Items will resize fluidly when
						// above this limit.
          move: 0, // {NEW} Integer: Number of carousel items that should move
					// on animation. If 0, slider will move all visible items.
          allowOneSlide: true, // {NEW} Boolean: Whether or not to allow a
								// slider comprised of a single slide

          // Callback API
          start: function() {}, // Callback: function(slider) - Fires when the
								// slider loads the first slide
          before: function() {}, // Callback: function(slider) - Fires
									// asynchronously with each slider animation
          after: function() {}, // Callback: function(slider) - Fires after each
								// slider animation completes
          end: function() {}, // Callback: function(slider) - Fires when the
								// slider reaches the last slide (asynchronous)
        ...
        Popover.prototype.sizePopover = function sizePopover() {
          ...
          if (popPosition === 'top' || popPosition === 'bottom') {
            ...
            if (popLeft + popWidth > winWidth) {
              ...
              // console.log('left %d, win %d, popw %d', popLeft, winWidth,
				// popWidth);
            }
    ...
    function(require, module, exports) {
      (function(global) {
        ...
        var Progress = (function() {
          ...
          /**
			 * Sets the progress bar status, where `n` is a number from `0.0` to
			 * `1.0`.
			 * 
			 * NProgress.set(0.4); NProgress.set(1.0);
			 */
          ...
          /**
			 * Shows the progress bar. This is the same as setting the status to
			 * 0%, except that it doesn't go backwards.
			 * 
			 * NProgress.start();
			 * 
			 */
          ...
          /**
			 * Hides the progress bar. This is the *sort of* the same as setting
			 * the status to 100%, with the difference being `done()` makes some
			 * placebo effect of some realistic motion.
			 * 
			 * NProgress.done();
			 * 
			 * If `true` is passed, it will show the progress bar even if its
			 * hidden.
			 * 
			 * NProgress.done(true);
			 */
          ...
          /**
			 * (Internal) renders the progress bar markup based on the
			 * `template` setting.
			 */
          ...
          /**
			 * (Internal) Applies css properties to an element, similar to the
			 * jQuery css method.
			 * 
			 * While this helper does assist with vendor prefixed property
			 * names, it does not perform any manipulation of values prior to
			 * setting styles.
			 */
    ...
    function(require, module, exports) {
      (function(global) {
        ...
        var SmoothScroll = function(element, options) {
          ...
          // temp variables (t will be a position between 0 and 1, y is the
          ...
          // when there's a touch detected while scrolling is in progress,
			// abort
          ...
          function render(now) {
            ...
            // only actually set scrollTop if there was a change fromt he last
			// frame
            if (lastY != y) {
              $this.scrollTop(y);
            }
      ...
      (function(global) {
        ...
        Sticky.prototype.checkPosition = function() {
          ...
          if (checkResult) {
            ...
            /*
			 * if (offsetBottom) { // （底部边距 + 元素高度 > 窗口高度） 时定位到底部 if
			 * ((offsetBottom + this.offset.height > $(window).height()) &&
			 * (scrollTop + $(window).height() >= scrollHeight - offsetBottom)) {
			 * $element.addClass(options.className.stickyBtm). css({top:
			 * $(window).height() - offsetBottom - this.offset.height}); } else {
			 * $element.removeClass(options.className.stickyBtm).css({top:
			 * offsetTop}); } }
			 */
          }
